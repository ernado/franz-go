...
func TestIssue1217(t *testing.T) {
	...
	t.Run("context_cancel_does_not_abort_poisoned_batch", func(t *testing.T) {
		...
		c.ControlKey(int16(kmsg.Produce), func(kreq kmsg.Request) (kmsg.Response, error, bool) {
			...
			// Block the retry until the test cancels the context.
			// SleepControl yields so other requests can proceed.
			c.SleepControl(func() { <-canceled })
			return nil, nil, false
		})
		...
		// Wait for the retry to arrive at the control function,
		// then cancel the context and unblock the retry.
		for attempt.Load() < 2 {
